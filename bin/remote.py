...
def run_sbatch_job(cmd, job_dir_name, job_name, time='01:00',
                   nodes=1, ntasks=1, cpus_per_task=1, mem_per_cpu='1GB', iterations=1):
...
def main(args):
    run_sbatch_job(args.cmd, args.job_dir, args.job_name, args.time,
                   args.nodes, args.ntasks, args.cpus_per_task, args.mem_per_cpu)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run commands from this /bin/ directory remotely.',
    ...
    parser.add_argument("--ntasks", type=int, default=1, help="How many processes do you want to "
                                                              "run?")
    parser.add_argument("--cpus-per-task", type=int, default=1, help="How many cpus per process?")
