...
def general_ax_settings(ax, ax_title='', xlabel=None, ylabel=None, xlabel_size=18,
                        ylabel_size=18, legend_label=None,
...
def histogram(cat, param, ax, bins=30, histtype='step', color='r', legend_label=None,
              vline=None,
              log_y=True,
    ...
    ax.hist(values, bins=bins, histtype=histtype, color=color, label=legend_label,
            **extra_hist_kwargs)
...
def binning3d_mass(cat, param1, param2, ax, ax_title=None,
...
def scatter_binning(cat, param1, param2, ax, nxbins=10, color='r', no_bars=False,
                    show_lines=False, show_bands=False,
    ...
    xqs = np.array(
        [[xmed - np.quantile(xbin, 0.25), np.quantile(xbin, 0.75) - xmed] for (xmed, xbin)
         in zip(xmeds, binned_x)]).T
    yqs = np.array(
        [[ymed - np.quantile(ybin, 0.25), np.quantile(ybin, 0.75) - ymed] for (ymed, ybin)
         in zip(ymeds, binned_y)]).T

    if not no_bars:
        ax.errorbar(xmeds, ymeds, xerr=xqs, yerr=yqs, fmt='o--', capsize=10, color=color,
                    label=legend_label)
    else:
        ax.errorbar(xmeds, ymeds, xerr=xqs, fmt='o-', color=color, label=legend_label,
                    capsize=10)
    ...
    general_ax_settings(ax, xlabel=xlabel, ylabel=ylabel, legend_label=legend_label,
                        **general_kwargs)
