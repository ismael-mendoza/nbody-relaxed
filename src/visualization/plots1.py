...
def generate_and_save(pdf, hcats, hplots, uplots, colors=None):
    if colors is None:
        colors = [None for _ in range(len(hcats))]

    for hcat, color, hplot in zip(hcats, colors, hplots):
        for plot in hplot:
            plot.generate(hcat.get_cat(), legend_label=hcat.catalog_label, color=color)
...
def plot_multiple_basic(hcats, pdf, colors):
    ...
    * (...)
    ...
    hist_kwargs2 = dict(bins=30, histtype='step', extra_hist_kwargs=dict(), log_y=True, vline='median',
                        **general_kwargs)
    ...
    plot1 = plots.UniPlot(plot_funcs.histogram, params, nrows=1, ncols=1, plot_kwargs=hist_kwargs)
    ...
    plot3 = plots.BiPlot(plot_funcs.scatter_binning, params, nrows=4, ncols=2, figsize=(18, 18),
    ...
    generate_and_save(pdf, hcats, hplots, uplots, colors=colors)
...
def plot_correlation_matrix_basic(hcats, pdf):
    ...
    for hcat, hplot in zip(hcats, hplots):
        ...
        plot = plots.MatrixPlot(stats.get_corrs, params, symmetric=False, plot_kwargs=kwargs,
...
def plot_decades_basic(hcats, pdf, colors):
    ...
    for hcat, hplot in zip(hcats, hplots):
        plot1 = plots.BiPlot(plot_funcs.binning3d_mass, params, nrows=4, ncols=4, figsize=figsize,
                             param_locs=param_locs, plot_kwargs=binning_3d_kwargs, title=hcat.catalog_label,
                             title_size=40)
        uplots.append(plot1)
        hplot.append(plot1)

    # Plot 6: Same plot as above but without any decades adn this one is overlaid for all hcats.
