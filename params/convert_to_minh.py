...
def get_catalogues(dir):
    ...
    if path.isfile(restart):
        with open(restart) as fp:
            target = fp.read().strip()
        if target == "done":
            return []
        i = hlists.index(target)
        if i == -1:
            raise ValueError("target '%s' in %s is not a valid hlist." % target, restart)
        hlists = hlists[i:]
...
def main():
    ...
    for sim in sims:
        ...
        for hlist in hlists:
            ...
            check_call(
                "./text_to_minh %s/config/minh.config all_vars.txt %s/haloes/raw/%s %s/haloes/minh_full"
                % (sim, sim, hlist, sim),
                shell=True,
            )
